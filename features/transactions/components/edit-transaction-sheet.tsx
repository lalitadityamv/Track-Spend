...
export const EditTransactionSheet = () => {
  ...
  // Fix: Add parentheses to actually call the onClose function
  const onSubmit = (values: FormValues) => {
    editMutation.mutate(values, {
      onSuccess: () => {
        onClose(); // Fixed: Call the function with parentheses
      },
    });
  };
        ...
        <SheetContent className="space-y-4">
          ...
          {transactionQuery.isError ? (
            <div className="p-4 text-destructive space-y-2">
              <div className="font-medium">Failed to load transaction</div>
              <div className="text-sm text-muted-foreground">
                {transactionQuery.error?.message || "Unknown error occurred"}
              </div>
            </div>
          ) : isLoading ? (
